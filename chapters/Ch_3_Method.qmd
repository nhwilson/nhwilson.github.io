...
mtcars %>%
  ...
  geom_point() +
  labs(title = "An Example of Negatively Correlated Data",
       subtitle = "From R's 'mtcars' dataset",
       caption = "If you're curious, the correlation coefficient is -0.77.") +
  xlab("Horsepower of a Car's Engine") +
  ylab("Fuel-Efficiency (MPG) of a Car's Engine") +
  theme_minimal()
...
Second, the correlations that we *can* observe are only as informative as the data collected in the first place.  This is true no matter what statistical wizardry is brought to bear---there is *no* foolproof way to "make up for" poorly-conducted surveys or a badly-formulated operationalization^[In this context, "operationalization" is just a fancy word for "taking a concept and then deciding how to concretely measure it."  For instance, in the figure above, we want to know how "fuel-efficient" a car is, but have a variety of different ways we could assess that.  We chose miles-per-gallon, and thus "operationalized" the concept of fuel-efficiency.] of a variable.

## Historical and Comparative Sociology

Last, and best, sociologists also use historical and comparative methods!  This family of methods uses information form the human past (whether published records, archival materials, oral histories, or even the writings of historians themselves) as the source of data for their investigations.  These data can span all of recorded human history, and often emphasize *comparing* at least two different phenomena to explain why their outcomes were different or similar to one another.^[If you want to know more, I wrote a book with a colleague about this!  See @mayrlComparisonPositivism2024.]
